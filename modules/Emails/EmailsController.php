    ...
    public function action_getFromFields()
    {
        ...
        foreach ($accounts as $inboundEmailId => $inboundEmail) {
            ...
            if (in_array($inboundEmail->id, $showFolders)) {
                ...
                $dataAddress = array(
                    ...
                    'id' => $inboundEmail->id,
                    'attributes' => array(
                        'reply_to' => utf8_encode($replyTo),
                        'name' => utf8_encode($fromName),
                        'from' => utf8_encode("$fromName &lt;$fromAddr&gt;"),
                    ),
        ...
        if ($oe->isAllowUserAccessToSystemDefaultOutbound()) {
            ...
            $data[] = array(
                ...
                'id' => $system->id,
                'attributes' => array(
                    'reply_to' => utf8_encode("$system->smtp_from_name &lt;$system->smtp_from_addr&gt;"),
                    'from' => utf8_encode("$system->smtp_from_name &lt;$system->smtp_from_addr&gt;"),
